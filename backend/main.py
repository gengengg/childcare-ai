...
【⚠ 문체 규칙 (다른 어떤 지시보다 우선. 반드시 지킬 것)】
- 종결 어미는 오직 **"-다체"(평서형 종결어미)** 만 사용한다.
  예: "관심을 보였다", "탐색하였다", "표현하였다", "즐거워하였다", "경험할 수 있었다".
- **금지 어미**: "-습니다", "-ㅂ니다", "-어요", "-아요", "-했어요", "-였어요",
  "-있었어요", "-보였어요", "-이에요", "-예요" 등 모든 존댓말/친근체.
- 매 문장의 끝을 확인하고, 존댓말이면 곧바로 다체로 고쳐 쓴다.
...
- **⚠ 분량 규칙 (반드시 지킬 것):
  - 각 요일 서술은 **한글 기준 380자 이상 500자 이하** (공백 포함 문자수).
  - 응답을 만들기 전에 각 요일별로 반드시 글자수를 세어 확인하고,
    380자 미만이면 놀이의 확장, 교사의 상호작용, 영아의 반응·표현·발달 경험을
    더 구체적으로 풀어 써서 380자를 채운 뒤 응답한다.
  - 500자를 초과하면 중요도 낮은 문장을 줄여 500자 이하로 조정한다.
  - 250~370자짜리 짧은 서술은 절대 허용되지 않는다.**
...
"""
    ...
    instructions = (
        ...
        "모든 문장은 반드시 '-다체'(평서형 종결어미)로 끝맺어야 해. "
        "'-습니다/-어요/-해요/-였어요' 같은 존댓말은 절대 쓰지 마. "
